...
parser.add_argument('-i', '--ipAddress', required=False, type=str,
                    default="192.168.10.5", help="The ip address of the device.")
...
myData = Data.Data()
yolo = RKNN_YOLO("obb_exported.float.rknn")
...
try:
    while True:
        if args.transport_protocol == "TCP":
            ...
            if myData.hasDepthMap:
                ...
                boxes = yolo.detect(adjusted_image)
                yolo.draw_result(adjusted_image, boxes)
                
                # 显示图像
                cv2.imshow('Depth Map', adjusted_image)
    ...
    cv2.destroyAllWindows()  # 确保在发生异常时也关闭窗口

deviceControl.login(Control.USERLEVEL_AUTH_CLIENT, '123456')
if args.transport_protocol == "TCP":
